...
class EnhancedStatisticsEngine:
    ...
    def __init__(self, match_history_window: int = 10, goal_diff_margin: int = 2):
    ...
    def compute_enhanced_rolling_stats(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["H_FormPts"] = self._rolling_sum(df, "Home", df["_H_Points"].fillna(0))
        df["A_FormPts"] = self._rolling_sum(df, "Away", df["_A_Points"].fillna(0))
        df.drop(columns=["_H_Points", "_A_Points"], inplace=True, errors="ignore")
    ...
    def compute_referee_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ref_stats = df.groupby("Referee").agg(
            ...
            ref_avg_yellows=(
                "HY",
                lambda x: (x + df.loc[x.index, "AY"]).mean()
                if "HY" in df.columns
                else 0,
            ),
        ...
        if "HY" in df.columns:
            ...
            df["Ref_Strictness"] = df["ref_avg_yellows"] / max(overall_avg_cards, 1)
        else:
    ...
    def context_to_features(ctx: MatchContext) -> Dict[str, float]:
        ...
        if ctx.home_squad:
            ...
            features["home_missing_key_count"] = len(
                ctx.home_squad.missing_key_players
            )
            features["home_available_key_count"] = len(
                ctx.home_squad.available_key_players
            )
        else:
            features.update(
                {
        ...
        if ctx.away_squad:
            ...
            features["away_missing_key_count"] = len(
                ctx.away_squad.missing_key_players
            )
            features["away_available_key_count"] = len(
                ctx.away_squad.available_key_players
            )
        else:
            features.update(
                {
        ...
        features["h2h_dominance"] = (ctx.h2h_home_wins - ctx.h2h_away_wins) / h2h_total
        ...
        if ctx.odds_home > 0:
            total_implied = 1 / ctx.odds_home + 1 / ctx.odds_draw + 1 / ctx.odds_away
        ...
        features["sentiment_diff"] = ctx.home_news_sentiment - ctx.away_news_sentiment
    ...
    def compute_poisson_features(
        ...
        for i in range(max_goals):
            for j in range(max_goals):
                prob_matrix[i, j] = poisson.pmf(i, home_lambda) * poisson.pmf(
                    j, away_lambda
                )
    ...
    def _rolling_sum(self, df: pd.DataFrame, group_col: str, values: pd.Series) -> pd.Series:
        """
        ...
        result = temp.groupby(group_col)["_val"].transform(
            lambda x: x.shift(1).rolling(window=self.n, min_periods=self.n).sum()
        )
        return result
...
class FeatureVectorBuilder:
    ...
    CONTEXT_FEATURES = [
        ...
        "home_injured_count",
        "away_injured_count",
        "home_missing_key_count",
        "away_missing_key_count",
        "ref_strictness",
        "ref_home_bias",
        "h2h_home_win_rate",
        "h2h_away_win_rate",
    ...
    def get_all_feature_names(self) -> List[str]:
        ...
        return self.ROLLING_FEATURES + self.ENHANCED_ROLLING + self.CONTEXT_FEATURES + self.POISSON_FEATURES
    ...
    def build_training_matrix(
        ...
        if feature_columns is None:
            feature_columns = [c for c in self.get_all_feature_names() if c in df.columns]
