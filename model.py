...
from config import (
    ...
    REFEREE_SETTINGS, KEY_NUMBERS, NEWS_SETTINGS
)
...
class MatchContext:
    ...
    home_attack_form: float = 0.0
    away_attack_form: float = 0.0
    home_defense_form: float = 0.0
    away_defense_form: float = 0.0
    home_card_risk: float = 0.0
    away_card_risk: float = 0.0
...
class ContextAdjuster:
    ...
    def apply_absence_impact(self, base_xg: float, impact: float) -> float:
        """Adjust xG based on total absence impact score."""
        impact = min(impact, NEWS_SETTINGS["max_absence_impact"])
        return base_xg * max(0.7, 1 - impact)

    def apply_form_factor(self, base_xg: float, form_adjustment: float) -> float:
        """Adjust xG based on attacking form signal."""
        return base_xg * max(0.7, 1 + form_adjustment)

    def apply_defense_form_factor(self, base_xg: float, defense_adjustment: float) -> float:
        """Adjust xG based on opponent defensive form signal."""
        return base_xg * max(0.7, 1 - defense_adjustment)

    def apply_card_risk_factor(self, base_xg: float, card_risk: float) -> float:
        """Adjust xG based on card/red card risk."""
        penalty = card_risk * NEWS_SETTINGS["card_risk_penalty"]
        return base_xg * max(0.7, 1 - penalty)
    ...
    def adjust_prediction(self, home_xg: float, away_xg: float, 
        ...
        home_xg = self.apply_absence_impact(home_xg, context.home_absence_impact)
        home_xg = self.apply_form_factor(home_xg, context.home_attack_form)
        home_xg = self.apply_card_risk_factor(home_xg, context.home_card_risk)
        ...
        away_xg = self.apply_absence_impact(away_xg, context.away_absence_impact)
        away_xg = self.apply_form_factor(away_xg, context.away_attack_form)
        away_xg = self.apply_card_risk_factor(away_xg, context.away_card_risk)

        # Defensive form affects opponent xG
        away_xg = self.apply_defense_form_factor(away_xg, context.home_defense_form)
        home_xg = self.apply_defense_form_factor(home_xg, context.away_defense_form)
...
class BettingModel:
    ...
    def _calculate_confidence(self, home_team: TeamStats, away_team: TeamStats,
        ...
        if context:
            ...
            if context.home_absence_impact > 0.15 or context.away_absence_impact > 0.15:
                confidence_score -= 10
