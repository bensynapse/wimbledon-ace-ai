...
class APIFootballCollector:
    ...
    def _request(self, endpoint: str, params: dict) -> Optional[dict]:
        ...
        try:
            ...
            if data.get("errors"):
                logger.warning("API error for %s: %s", endpoint, data["errors"])
                return None

            self._cache[cache_key] = data
            remaining = resp.headers.get("x-ratelimit-requests-remaining", "?")
            logger.debug("API call %s - remaining: %s", endpoint, remaining)
    ...
    def get_player_stats(self, team_id: int, season: int, league_id: int) -> List[PlayerInfo]:
    ...
    def get_h2h(self, home_team_id: int, away_team_id: int, last_n: int = 10) -> Dict[str, Any]:
        """Get head-to-head record between two teams."""
        data = self._request(
            "fixtures/headtohead",
            {"h2h": f"{home_team_id}-{away_team_id}", "last": last_n},
        )
        if not data or not data.get("response"):
            return {"home_wins": 0, "away_wins": 0, "draws": 0, "avg_goals": 0, "matches": 0}
    ...
    def get_predicted_lineups(self, fixture_id: int) -> Dict[str, Any]:
        ...
        for lineup in lineups if isinstance(lineups, list) else []:
            ...
            result[team_name] = {
                ...
                "starting_xi": [
                    p.get("player", {}).get("name") for p in lineup.get("startXI", [])
                ],
                "substitutes": [
    ...
    def get_team_xg(
        self, team_id: int, league_id: int, season: int, last_n: int = 10
    ) -> Dict[str, float]:
        ...
        n = max(count, 1)
        return {"xg_for": round(xg_for_total / n, 2), "xg_against": round(xg_against_total / n, 2)}
...
class OddsCollector:
    ...
    def get_match_odds(
        self, sport: str = "soccer_epl", regions: str = "eu", markets: str = "h2h,totals"
    ) -> List[Dict]:
                        ...
                        for outcome in market.get("outcomes", []):
                            if outcome.get("point") == 2.5:
                                if outcome["name"] == "Over":
                                    best["over25"] = max(best["over25"], outcome["price"])
                                else:
                                    best["under25"] = max(best["under25"], outcome["price"])
        return best
...
class NewsSentimentCollector:
    ...
    def get_team_sentiment(self, team_name: str, days_back: int = 3) -> float:
        """
        ...
        try:
            ...
            logger.warning("News API error for %s: %s", team_name, exc)
            return 0.0
...
class HistoricalDataCollector:
    ...
    def download_league(
        self, league_code: str, country_code: str, start_year: int = 2018
        ...
        for year in range(start_year, date.today().year + 1):
            ...
            try:
                ...
                df["Season"] = year
                dfs.append(df)
                logger.info("Downloaded %s season %s", league_code, year)
            except Exception as exc:
                logger.debug("No data for %s %s: %s", league_code, year, exc)
                continue
        ...
        available = {c: self.COLUMN_MAP[c] for c in self.COLUMN_MAP if c in combined.columns}
        combined = combined.rename(columns=available)

        # Keep all available columns
        keep_cols = ["Date", "Season"] + [
            v for v in available.values() if v not in ("Date", "Season")
        ]
...
class MatchDataAggregator:
    ...
    def __init__(self, api_football_key: str = "", odds_api_key: str = "", news_api_key: str = ""):
        self.api_football = APIFootballCollector(api_football_key) if api_football_key else None
    ...
    def build_match_context(
        ...
        if self.api_football:
            # Injuries
            injuries = self.api_football.get_injuries(league_id, season, fixture_id)
            home_injuries = [p for p in injuries if p.team.lower() in home_team.lower()]
            away_injuries = [p for p in injuries if p.team.lower() in away_team.lower()]

            # Player stats
            home_players = self.api_football.get_player_stats(home_team_id, season, league_id)
            away_players = self.api_football.get_player_stats(away_team_id, season, league_id)

            # Mark key players (top 5 by goals+assists)
            for players in [home_players, away_players]:
                players.sort(
                    key=lambda p: p.goals_season + p.assists_season, reverse=True
                )
                for p in players[:5]:
                    p.is_key_player = True

            # Build squad status
            ctx.home_squad = self._build_squad_status(home_team, home_players, home_injuries)
            ctx.away_squad = self._build_squad_status(away_team, away_players, away_injuries)

            # Referee
            ctx.referee = self.api_football.get_fixture_referee(fixture_id)
            ...
            # xG
            home_xg = self.api_football.get_team_xg(home_team_id, league_id, season)
            away_xg = self.api_football.get_team_xg(away_team_id, league_id, season)
    ...
    def _build_squad_status(
        self, team_name: str, players: List[PlayerInfo], injuries: List[PlayerInfo]
    ) -> TeamSquadStatus:
        ...
        available_key = [p for p in players if p.is_key_player and p.is_available]
        missing_key = [p for p in players if p.is_key_player and not p.is_available]
        ...
        for p in missing_key:
            ...
            contribution = (p.goals_season + p.assists_season) / max(
                sum(pl.goals_season + pl.assists_season for pl in players if pl.is_key_player),
                1,
            )
