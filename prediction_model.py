...
class EnsemblePredictor:
    ...
    def train(
        ...
        for name, model in self.models.items():
            ...
            logger.info(
                "  %s: CV=%.4f ± %.4f", name, cv_scores.mean(), cv_scores.std()
            )
...
class ValueBetAnalyzer:
    ...
    def analyze_match(
        ...
        if poisson_probs:
            ...
            prob_h = (1 - poisson_weight) * prob_h + poisson_weight * poisson_probs.get(
                "poisson_home_win", prob_h
            )
            prob_d = (1 - poisson_weight) * prob_d + poisson_weight * poisson_probs.get(
                "poisson_draw", prob_d
            )
            prob_a = (1 - poisson_weight) * prob_a + poisson_weight * poisson_probs.get(
                "poisson_away_win", prob_a
            )
    ...
    def _analyze_key_factors(
        ...
        if abs(xg_sup) > 0.3:
            ...
            factors.append(f"{better} has xG superiority ({xg_sup:+.2f})")
        ...
        if odds.get("home", 0) > 0:
            ...
            if odds.get("away", 0) > 0 and probs[2] > 1 / odds.get("away", 99) + 0.08:
                factors.append(
    ...
    def _assess_risk(self, confidence: float, value: float, ctx: Dict[str, float]) -> str:
        ...
        total_missing = ctx.get("home_missing_key_count", 0) + ctx.get(
    ...
    def filter_value_bets(self, recommendations: List[BetRecommendation]) -> List[BetRecommendation]:
        """
        Filter recommendations to only include genuine value bets.
        """
        value_bets = []
        for rec in recommendations:
            if rec.best_value >= self.min_value and rec.confidence >= self.conf_threshold:
